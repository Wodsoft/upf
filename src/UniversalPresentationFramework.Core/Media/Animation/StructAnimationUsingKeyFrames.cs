...
{
    public abstract class StructAnimationUsingKeyFrames<TValue, TKeyFrame, TKeyFrameCollection> : GenericAnimationBase<TValue>, IAddChild
        where TValue : struct
        where TKeyFrame : GenericAnimationKeyFrame<TValue>
        where TKeyFrameCollection : KeyFrameCollection<TKeyFrame>, IHaveEmpty<TKeyFrameCollection>, new()
    {
        ...
        /// <Summary>
        /// Creates a new StructAnimationUsingKeyFrames.
        /// </Summary>
        public StructAnimationUsingKeyFrames()
            : base()
        {
        ...
        protected override bool FreezeCore(bool isChecking)
        {
            ...
            canFreeze &= Freezable.Freeze(KeyFrames, isChecking);
        ...
        {
            StructAnimationUsingKeyFrames<TValue, TKeyFrame, TKeyFrameCollection> sourceAnimation = (StructAnimationUsingKeyFrames<TValue, TKeyFrame, TKeyFrameCollection>)sourceFreezable;
        ...
        {
            StructAnimationUsingKeyFrames<TValue, TKeyFrame, TKeyFrameCollection> sourceAnimation = (StructAnimationUsingKeyFrames<TValue, TKeyFrame, TKeyFrameCollection>)source;
        ...
        private void CopyCommon(StructAnimationUsingKeyFrames<TValue, TKeyFrame, TKeyFrameCollection> sourceAnimation, bool isCurrentValueClone)
        {
            ...
            if (sourceAnimation._keyFrames != null)
            {
                ...
                {
                    _keyFrames = (TKeyFrameCollection)sourceAnimation._keyFrames.CloneCurrentValue();
                }
                else
                {
                    _keyFrames = (TKeyFrameCollection)sourceAnimation._keyFrames.Clone();
                }
        ...
        {
            if (child is TKeyFrame keyFrame)
            {
        ...
        {
            throw new NotSupportedException("Not support text child.");
        }
        ...
        /// </returns>
        protected override TValue GetCurrentValueCore(
            TValue defaultOriginValue,
            TValue defaultDestinationValue,
            AnimationClock animationClock)
        {
            ...
            else
            {
                ...
                float currentSegmentProgress = 0f;
                TValue fromValue;
                ...
                if (currentResolvedKeyFrameIndex == 0)
                {
                    ...
                    if (IsAdditive)
                    {
                        fromValue = GetZeroValue(defaultOriginValue);
                    }
                    else
                    {
                    ...
                    currentSegmentProgress = (float)(currentTime.TotalMilliseconds
                ...
                else
                {
                    ...
                    currentSegmentProgress = (float)(segmentCurrentTime.TotalMilliseconds
            ...
            if (IsCumulative)
            {
                float currentRepeat = (float)(animationClock.CurrentIteration - 1);

                if (currentRepeat > 0.0)
                {
                    currentIterationValue = Add(
                        currentIterationValue,
                        Scale(GetResolvedKeyFrameValue(maxKeyFrameIndex), currentRepeat));
                }
            }
            ...
            if (IsAdditive)
            {
                return Add(defaultOriginValue, currentIterationValue);
            }
        ...
        public TKeyFrameCollection KeyFrames
        {
            get
            {
                ...
                if (_keyFrames == null)
                {
                    if (this.IsFrozen)
                    {
                        _keyFrames = TKeyFrameCollection.Empty;
                    }
                    else
                    {
        ...
        private TValue GetResolvedKeyFrameValue(Int32 resolvedKeyFrameIndex)
        {
        ...
        private TKeyFrame GetResolvedKeyFrame(Int32 resolvedKeyFrameIndex)
        {
        ...
        private void ResolvePacedKeyTimes()
        {
            ...
            do
            {
                if (_keyFrames[index].KeyTime.Type == KeyTimeType.Paced)
                {
                    ...
                    TValue prevKeyValue = _keyFrames[index - 1].Value;

                    do
                    {
                        TValue currentKeyValue = _keyFrames[index].Value;

                        // Determine the segment length for this key frame and
                        // add to the total length.
                        totalLength += GetSegmentLength(prevKeyValue, currentKeyValue);
                    ...
                    totalLength += GetSegmentLength(prevKeyValue, _keyFrames[index].Value);
        ...
        protected abstract float GetSegmentLength(TValue from, TValue to);

        protected abstract TValue GetZeroValue(TValue value);

        protected abstract TValue Add(TValue value1, TValue value2);

        protected abstract TValue Scale(TValue value, float factor);
