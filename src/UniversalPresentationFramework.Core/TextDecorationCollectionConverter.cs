...
namespace Wodsoft.UI
{
    public sealed class TextDecorationCollectionConverter : TypeConverter
    {
        ...
        {
            if (input == null || input is not string value)
        ...
        public static new TextDecorationCollection ConvertFromString(string text)
        {
            ...
            {
                if (Match(_None, text, index))
                {
                    // Matched "None" in the input
                    index = AdvanceToNextNonWhiteSpace(text, index + _None.Length);
                ...
                else
                {
                    ...
                    int i;
                    for (i = 0;
                           i < _TextDecorationNames.Length
                        && !Match(_TextDecorationNames[i], text, index);
                       i++
                    ) ;

                    if (i < _TextDecorationNames.Length)
                    {
                        ...
                        else
                        {
                            ...
                            // has been added
                            foreach (TextDecoration item in _PredefinedTextDecorations[i])
                                textDecorations.Add(item);
                            matchedTextDecorationFlags |= (byte)(1 << i);

                            // Advance to the start of next name
                            index = AdvanceToNextNameStart(text, index + _TextDecorationNames[i].Length);
                        }
                    }
                    else
                    {
            ...
            if (index < 0)
            {
                throw new ArgumentException("Invalid TextDecorationCollection string.");
            }
        ...
        private static readonly string[] _TextDecorationNames = [
            "OVERLINE",
            "BASELINE",
            "UNDERLINE",
            "STRIKETHROUGH"
        ...
        private static readonly TextDecorationCollection[] _PredefinedTextDecorations =
