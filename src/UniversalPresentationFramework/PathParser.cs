...
namespace Wodsoft.UI
{
    ...
    internal class PathParser
    {
        ...
        public SourceValueInfo[] Parse(string path)
        {
            ...
            {
                char c = (_index < _n) ? _path[_index] : _NullChar;
                    ...
                    case State.DrillIn:
                        switch (c)
                        {
                            ...
                            case _NullChar:
                                break;
                            default:
                                SetError($"Path syntax error at {_index} with \"{_path}\".");
                                return _EmptyInfo;
                        }
                        _state = State.Prop;
                        break;
        ...
        void AddProperty()
        {
            ...
            if (level > 0)
            {
                SetError($"Unmatched parent at {start} with \"{_path}\".");
                return;
            }
        ...
        void AddIndexer()
        {
            ...
            List<IndexerParamInfo> paramList = new List<IndexerParamInfo>();
            ...
            while (state != IndexerState.Done)
            {
                if (_index >= _n)
                {
                    SetError($"Unmatched bracket at {start - 1} with \"{_path}\".");
                    return;
                }
        ...
        const char _NullChar = Char.MinValue;
        const char _EscapeChar = '^';
        static SourceValueInfo[] _EmptyInfo = Array.Empty<SourceValueInfo>();
    }
}
