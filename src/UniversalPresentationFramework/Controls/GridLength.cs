...
namespace Wodsoft.UI.Controls
{
    ...
    public struct GridLength : IEquatable<GridLength>
    {
        ...
        public GridLength(float value, GridUnitType type)
        {
            ...
            {
                throw new ArgumentException("Value can not be NaN.", "value");
            }
            if (float.IsInfinity(value))
            {
                throw new ArgumentException("Value can not be infinity", "value");
            }
            if (type != GridUnitType.Auto
                && type != GridUnitType.Pixel
                && type != GridUnitType.Star)
            {
                throw new ArgumentException("Unknown grid unit type.", "type");
            }

            _unitValue = (type == GridUnitType.Auto) ? 0.0f : value;
            _unitType = type;
        }
        ...
        /// </summary>
        public float Value { get { return ((_unitType == GridUnitType.Auto) ? 1.0f : _unitValue); } }
        ...
        private static readonly GridLength _Auto = new GridLength(1.0f, GridUnitType.Auto);
